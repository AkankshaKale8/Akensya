...
def topbar(title, subtitle):
    ...
    # Use Streamlit's image renderer instead of a raw local-file <img> URL.

    # Raw filesystem paths are not publicly served by Streamlit Cloud, which

    # is why the previous header showed a broken-image icon.

    st.markdown('<div class="ak-top">', unsafe_allow_html=True)

    logo_col, text_col = st.columns([0.12, 0.88], vertical_alignment="center")

    with logo_col:

        st.image(str(ASSET), width=68)
...
# A selectbox is more reliable than the earlier custom radio navigation on

# mobile browsers and across Streamlit Cloud reruns. It also makes it clear

# that every selection is a full application page.
